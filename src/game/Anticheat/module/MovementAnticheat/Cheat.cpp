...
bool PlayerCheatData::HandleAnticheatTests(MovementInfo& movementInfo, WorldSession* session, WorldPacket* packet)
{
    ...
    if (!sCheatsMgr->EnableAnticheat() || me != session->GetPlayer() || (me->GetSession()->GetSecurity() != SEC_PLAYER))
        return true;
...
bool PlayerCheatData::CheckTeleport(uint32 opcode, MovementInfo& movementInfo)
{
    ...
    if (!sCheatsMgr->EnableAnticheat() || (me->GetMover()->GetTypeId() != TYPEID_PLAYER) || (me->GetSession()->GetSecurity() != SEC_PLAYER))
        return true;
